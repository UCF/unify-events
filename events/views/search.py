...
class GlobalSearchView(MultipleFormatTemplateViewMixin, ListView):
    ...
    def get_queryset(self):
        ...
        query_split = query.split(' ') if query else None
        if query:
            now = datetime.now().replace(hour=0, minute=0, second=0)

            qs_filter = Q(title__icontains=query) | Q(calendar__title__icontains=query)

            for q in query_split:
                qs_filter |= Q(tags__name__istartswith=q)



            queryset = Event.objects.filter(qs_filter
